...
- **自动化行动**：插件把以下能力注册为 ClassIsland 自动化行动，可在「自动化」中按任意条件（时间、课程、天气等）触发：
  - **切换用户预设**（`miku.classisland.injector.switchPreset`）——按条件自动切换整套方案，这是把“满足条件即切换预设方案”落到自动化的入口；
  - **应用样式预设** / **应用动画预设**——套用内置预设；
  - **修改设置**（`miku.classisland.injector.setSetting`）——插件所有设置项都拆成了独立的自动化行动，如“不透明度”“动画类型”“背景色”等。
