            ...
            [
                ...
                Spec("RippleType", "提醒 Ripple", "\uEFFF", SettingValueKind.Enum,
                    ...
                    [KV(RippleType.None, "无"), KV(RippleType.Ring, "单环"), KV(RippleType.DoubleRing, "双环"),
                     ...
                     KV(RippleType.Cinematic, "屏幕涟漪"), KV(RippleType.Pjsk, "pjsk 强调")]),
                Spec("PjskRippleDirection", "pjsk 特效方向", "\uEFFF", SettingValueKind.Enum,
                    "pjsk 强调特效相对判定线（主界面）的喷射方向。", typeof(PjskRippleDirection),
                    [KV(PjskRippleDirection.Up, "向上（原版）"), KV(PjskRippleDirection.Down, "向下（镜像）")]),
                Spec("PjskNoteStyle", "pjsk note 效果样式", "\uEFFF", SettingValueKind.Enum,
                    "note 击打效果的配色样式。", typeof(PjskNoteStyle),
            ...
            [
                ...
                Spec("BackgroundTextureType", "背景纹理", "\uE92B", SettingValueKind.Enum,
                    ...
                    [KV(BackgroundTexture.None, "无"), KV(BackgroundTexture.Grid, "网格线"),
                     ...
                     KV(BackgroundTexture.Aero, "Aero 玻璃条纹")]),
                ...
                Spec("TextStylingEnabled", "文字美化", "\uE8D2", SettingValueKind.Bool,
                    "美化主界面文字：字体 / 字号 / 字色 / 勾边。"),
                Spec("TextFontFamily", "字体", "\uE8D2", SettingValueKind.String,
                    "字体名称（如 Microsoft YaHei）；留空保持宿主原字体。"),
                Spec("TextFontSize", "字号", "\uE8D2", SettingValueKind.Double, 0, 96, "0",
                    "主界面标准文字的字号（像素），0 = 保持宿主原字号。"),
                Spec("TextFontScale", "字号缩放", "\uE8D2", SettingValueKind.Double, 0.3, 3, "0.##",
                    "未指定字号时按此比例整体缩放宿主原字号（1 = 不缩放）。"),
                Spec("TextColorMode", "字色来源", "\uE8D2", SettingValueKind.String,
                    "KeepHost / Fixed / AutoInvert（跟随背景自动反色）。"),
                Spec("TextColor", "固定字色", "\uE8D2", SettingValueKind.String,
                    "固定字色模式下的文字颜色（如 #FFFFFFFF）。"),
                Spec("TextLightColor", "深色背景字色", "\uE8D2", SettingValueKind.String,
                    "自动反色：背景偏暗时使用的字色。"),
                Spec("TextDarkColor", "浅色背景字色", "\uE8D2", SettingValueKind.String,
                    "自动反色：背景偏亮时使用的字色。"),
                Spec("TextInvertThreshold", "反色阈值", "\uE8D2", SettingValueKind.Double, 0, 1, "0.##",
                    "背景相对亮度高于该值时使用浅色背景字色（0.55 约为中间灰）。"),
                Spec("TextOutlineThickness", "勾边宽度", "\uE8D2", SettingValueKind.Double, 0, 8, "0.##",
                    "文字轮廓宽度，0 = 不勾边。"),
                Spec("TextOutlineColor", "勾边颜色", "\uE8D2", SettingValueKind.String,
                    "文字轮廓颜色（如 #CC000000）。"),
                Spec("TextStylingMainWindowOnly", "仅主界面生效", "\uE8D2", SettingValueKind.Bool,
                    "只美化主界面文字，提醒等其它区域保持宿主原样。"),
            ]
        },
        new()
        {
            ...
            [
                Spec("WallpaperEnabled", "主界面底图", "\uF42D", SettingValueKind.Bool,
                    "层级：底图 → 底色 → 组件。"),
                Spec("WallpaperBlurRadius", "底图模糊", "\uF42D", SettingValueKind.Double, 0, 60, "0.##",
                    "对整个底图宿主应用高斯模糊（0 为关闭）。"),
            ]
        },
        new()
