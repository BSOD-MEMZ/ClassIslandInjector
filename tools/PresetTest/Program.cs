...
Check("全新默认 动态主题色=false", false, fresh.DynamicThemeColorEnabled);
Check("全新默认 鼠标悬停保持可见=false", false, fresh.MouseHoverKeepVisible);
Check("全新默认 点击特效=false", false, fresh.ClickEffectEnabled);
Check("全新默认 点击特效类型=扩散圆环", ClickEffectType.Ring, fresh.ClickEffectType);
Check("全新默认 虚假天气=false", false, fresh.FakeWeatherEnabled);
Check("全新默认 虚假天气温度=25", 25.0, fresh.FakeWeatherTemperature);
Check("全新默认 虚假天气湿度=40", 40.0, fresh.FakeWeatherHumidity);
Check("全新默认 虚假天气 AQI=50", 50.0, fresh.FakeWeatherAqi);
Check("全新默认 预警图标=无", 0, fresh.FakeWeatherAlertIcon);
Check("全新默认 预警类型=空", "", fresh.FakeWeatherAlertType);
